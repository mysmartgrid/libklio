...
void CSVStore::remove_db(const std::string& db_path) {

    bfs::remove_all(db_path);
}

void CSVStore::save_sensor(const bool create, const Sensor::Ptr sensor) {

    std::ofstream file(compose_sensor_path(sensor->uuid_string()));
